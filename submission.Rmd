...
In order to be able to reproduce meta-analyses of aggregate data, primary-level^[By primary-level data we mean aggregate data from included primary studies.] effects sizes and their associated standard errors are required. These are generally computed from statistics retrieved from the primary studies such as means, standard deviations or sample sizes. We attempted to retrieve the least processed data \textcolor{blue}{shared by the authors of the meta-analysis.} First, we sought for the statistics used to compute primary effect sizes (e.g., means, sd); second, we sought for the primary effects sizes already computed and their standard errors (or, alternatively, the sampling variances): finally, we sought for the primary effects sizes and their confidence limits, from which the standard errors were approximated as follows: 
...
Figure 3 summarizes the primary data retrieval results. Based on the availability of primary data, either retrieved directly from the paper or upon request, `r df_process_rep$n[2]` meta-analyses (`r sprintf("%0.0f%%", df_process_rep$perc[2]*100)`, see Fig. 3a) were labelled as process reproducible. \textcolor{blue}{Additionally, as the time span covered is fairly wide, the process reproducible rate was also computed for different time periods. The meta-analyses were grouped into five-year periods, except for the initial period, which was grouped into a ten-year period due to the limited number of meta-analyses available during the first five-year period, which consisted of only five meta-analyses. The process reproducibility rate was }`r sprintf("%0.0f%%, (%0.0f/%0.0f)", df_process_rep2$perc[2]*100, df_process_rep2$n[2], sum(df_process_rep2$n))`, `r sprintf("%0.0f%%, (%0.0f/%0.0f)", df_process_rep3$perc[2]*100, df_process_rep3$n[2], sum(df_process_rep3$n))`, and `r sprintf("%0.0f%%, (%0.0f/%0.0f)", df_process_rep4$perc[2]*100, df_process_rep4$n[2], sum(df_process_rep4$n))` \textcolor{blue}{for meta-analyses published between 2000 and 2010, 2011 and 2015, and 2016 and 2020, respectively (see Fig. 3a). This trend is further explored in the Supplementary file available at: https://osf.io/fjhpw.}

Of these `r df_process_rep$n[2]` meta-analyses, in about half of the cases the primary data was retrieved from a forest plot in the paper itself and in about a third of the cases the primary data was retrieved from supplementary files (see Fig. 3b for further details). 
...
In most cases, when the meta-analytic data was available, it was shared in document formats. Data were retrieved from tables or forest plots in *pdf* or *docx* format—either in the document itself or in the supplementary materials—in `r sprintf("%0.0f%% (%0.0f/%0.0f)", (df_source_primary_data$perc[3]+df_source_primary_data$perc[4]+df_source_primary_data$perc[6])*100, df_source_primary_data$n[3]+df_source_primary_data$n[4]+df_source_primary_data$n[6], sum(df_source_primary_data$n))` of the cases. This required a manual recoding of the primary data to be able to reuse them. Furthermore, when data was reported through general tables (i.e. tables listing all the primary studies included with their characteristics), the meta-analysis associated with each data entry was not always obvious, leading to the time-consuming task of matching each data entry with each independent meta-analytic result reported in the paper. There were only `r df_source_primary_data$n[1]` meta-analyses (from `r words(sum(df_data_downloaded$Item))` different papers) of the `r df_process_rep$n[2]` meta-analyses labelled as process reproducible (`r sprintf("%0.0f%%", df_source_primary_data$perc[1]*100)`), where the task of retrieving the data required simply downloading the data in an machine-readable data file format.
...
``` {r, fig.width = 10, fig.height=13, fig.cap="Percentage of (a) process-reproducible meta-analyses; (b) different types of sources of original data; (c) data request results."}
